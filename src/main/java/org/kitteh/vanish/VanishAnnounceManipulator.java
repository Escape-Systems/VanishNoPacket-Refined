...
/**
 * Controller of announcing joins and quits that aren't their most honest. Note that delayed
 * announce methods can be called without checking to see if it's enabled first. The methods confirm
 * before doing anything particularly stupid.
 */
public final class VanishAnnounceManipulator {
  ...
  void vanishToggled(@NonNull Player player) {
    if (Settings.getAutoFakeJoinSilent() && this.delayedAnnouncePlayerList.contains(
        player.getName())) {
